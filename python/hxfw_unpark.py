...
def compile():
    subprocess.check_call(r'/opt/maven/bin/mvn install:install-file -Dfile=./commons/commons-util/libs/webp-imageio-core-0.1.3.jar -DgroupId=com.github.nintha -DartifactId=webp-imageio-core -Dversion=0.1.3 -Dpackaging=jar', cwd=work_space_path, shell=True)
    commons_path = os.path.join(work_space_path, 'commons')
    subprocess.check_call('/opt/maven/bin/mvn clean install -Dmaven.test.skip=true', cwd=commons_path, shell=True)
    admin_api_path = os.path.join(work_space_path, 'admin-api')
    subprocess.check_call('/opt/maven/bin/mvn clean package -Ptest -Dmaven.test.skip=true', cwd=admin_api_path, shell=True)
